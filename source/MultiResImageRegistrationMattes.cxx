...
int main( int argc, char *argv[] )
{
  if( argc < 4 )
    ...
    std::cerr << " outputImagefile [checkerBoardBefore]";
    std::cerr << " [checkerBoardAfter] [differenceBeforeRegistration]";
    std::cerr << " [differenceAfterRegistration][numberOfBins]"<< std::endl;
	  std::cerr << " [numberOfIterations] [relaxationFactor]" << std::endl;
    std::cerr << " [Resolution levels] " << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if( argc > 10 ){
	  optimizer->SetRelaxationFactor( atof(argv[10] ));
  }
  ...
  if( argc > 11 ){
    registration->SetNumberOfLevels( atoi(argv[11] ));
  }
